        ...
        Config.worlds.forEach((world) => {
            if (typeof world.status_port == 'number' && world.status_port > 0) {
                if (used_ports[world.status_port]) {
                    throw `Duplicated status port.\nStatus port for world ${
                        world.name
                    } is the same as status port for world ${
                        used_ports[world.status_port]
                    }.`;
                }
...
function loadConfig(dir: string): boolean {
    try {
        ...
        const worldsDir = path.resolve(`${dir}/worlds`);
        const files = fs.readdirSync(worldsDir);
        for (let i = 0; i < files.length; ++i) {
            const file = files[i];
            if (file.toLowerCase().indexOf('.json') < 1) continue;
            const world = require(path.resolve(`${worldsDir}/${file}`));
