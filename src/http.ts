...
import Vocations from './services/vocations';
...
export default class TibiaHTTP {
    start = (app: TemplatedApp) => {
        app.options('/api', (res, req) => {
            ...
            .any('/api', (res: HttpResponse, req: HttpRequest) => {
                if (
                    !Limits.acceptConnection(
                        Buffer.from(res.getRemoteAddress()),
                    )
                ) {
                    return res.close();
                }
    ...
    private statusUrl = async (res: HttpResponse, req: HttpRequest) => {
        ...
        const status = await Status.getCached(0);
        if (!aborted) {
            res.end(status);
        }
    };
    ...
    private eventSchedule = (res: HttpResponse, data: any) => {
        const response = {
            eventlist: [
                {
                    description:
                        'The moon is full! Beware, lycanthropic creatures like werewolves, werefoxes or werebears roam the lands now. And they are more aggressive and numerous than usual.',
                    startdate: Math.floor(Date.now() / 1000 + 100),
                    colordark: '#735D10',
                    name: 'Full Moon',
                    enddate: Math.floor(Date.now() / 1000 + 88100),
                    isseasonal: false,
                    colorlight: '#8B6D05',
                },
                {
                    description:
                        'Winterberries can now be found all over Tibia! The Combined Magical Winterberry Society wants YOU to help gathering them to create the juice that keeps the magic in the world flowing - good treading!',
                    startdate: Math.floor(Date.now() / 1000 + 200),
                    colordark: '#7A4C1F',
                ...
                {
                    description:
                        'This is the time of witches, ghosts and vampires. Look out for the Mutated Pumpkin and the Halloween Hare to make your flesh crawl.',
    ...
    private login = async (res: HttpResponse, data: any) => {
        ...
        const ip_address = Buffer.from(res.getRemoteAddress());
        const account_name = data.accountname;
        const account_password = data.password;
        const account_token = data.token || '';
        const stayloggedin = data.stayloggedin;

        // codes: 3 - invalid acc/pass, 6 - token is required
        const loginError = (message: string, code = 3) => {
            if (aborted) return;
            return res.end(
                JSON.stringify({
                    errorCode: code,
        ...
        const account = await DB.loadAccountByName(account_name);
        ...
        characters.forEach((character) => {
            response['playdata']['characters'].push({
                ...
                vocation: Vocations.getVocationNameByID(character.vocation),
                tutorial: false,
