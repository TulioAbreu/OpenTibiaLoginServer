...
class Status {
    ...
    getCached = async (world_id: number) => {
        if (
            !this.cache[world_id] ||
            this.cache[world_id].lastUpdate + UPDATE_INTERVAL < Date.now()
        ) {
            this.cache[world_id] = {
    ...
    private get = async (world_id: number) => {
        const world = Config.worlds.get(world_id);
        if (!world) {
            return 'INVALID_WORLD_ID';
        }

        const playersOnline = await DB.getPlayersOnline(world_id);
        const playersOnlinePeak = Math.max(
            playersOnline + 1,
            await DB.getOnlineRecord(world_id),
        );
