...
class Limits {
    ...
    acceptConnection = (address: number | string | Buffer): boolean => {
        ...
        if (!this.connectionLimits.has(addr)) {
            this.connectionLimits.set(addr, {
                t: new Array<number>(
                    Config.limits.connections.interval.length,
                ).fill(0),
                c: new Array<number>(
                    Config.limits.connections.interval.length,
                ).fill(0),
        ...
        const limit = this.connectionLimits.get(addr);
        for (let i = 0; i < limit.c.length; ++i) {
            if (
                limit.t[i] + Config.limits.connections.interval[i] <
                Date.now() / 1000
            ) {
    ...
    acceptAuthorization = (address: number | string | Buffer): boolean => {
        ...
        const limit = this.authLimits.get(addr);
        if (!limit) {
            return true;
        }
        ...
        for (let i = 0; i < limit.c.length; ++i) {
            if (
                limit.t[i] + Config.limits.authorizations.interval[i] <
                Date.now() / 1000
            ) {
                limit.t[i] = Date.now() / 1000;
                limit.c[i] = 0;
    ...
    addInvalidAuthorization = (address: number | string | Buffer) => {
        ...
        if (!this.authLimits.has(addr)) {
            this.authLimits.set(addr, {
                t: new Array<number>(
                    Config.limits.authorizations.interval.length,
                ).fill(0),
                c: new Array<number>(
                    Config.limits.authorizations.interval.length,
                ).fill(0),
